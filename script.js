...
function setupControls(){
  ...
  $('resetTextBtn').addEventListener('click',()=>{
    settings.message.size=100;
    settings.name.size=60;
    syncControls();
    drawCard();
  });
...
function pointerDown(e){
  ...
  const dm=Math.hypot(p.x-boxes.message.x,p.y-boxes.message.y);
  const dn=Math.hypot(p.x-boxes.name.x,p.y-boxes.name.y);
  const messageAvailable=messageInput.value.trim().length>0;
  const nameAvailable=nameInput.value.trim().length>0;
  let target=null;
  if(messageAvailable && nameAvailable) target=dm<=dn?'message':'name';
  else if(messageAvailable) target='message';
  else if(nameAvailable) target='name';
  if(!target || Math.min(dm,dn)>Math.max(boxes[target].r,60))return;
